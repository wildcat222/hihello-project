...
public class HRController {
    ...
    public ApiResponse<?> modifyEmployee(@PathVariable Long employeeSeq,
                                         @RequestBody ModifyEmplReqDTO modifyEmplReqDTO) {

        hrService.modifyEmpl(employeeSeq, modifyEmplReqDTO);
        return ResponseUtil.successResponse("사원 정보가 수정되었습니다.").getBody();
    }

    @Operation(summary = "사원 정보 삭제", description = "담당자가 사원을 삭제한다.")
    @DeleteMapping("/user/{employeeSeq}")
    public ApiResponse<?> deleteEmployee(@PathVariable Long employeeSeq) {

        hrService.deleteEmpl(employeeSeq);
        return ResponseUtil.successResponse("사원이 삭제되었습니다.").getBody();
    }
}
